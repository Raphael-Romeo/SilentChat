...
let minimum_side_panel_width = 180;
let compact_side_panel_mode = false;


if (window.innerWidth < 800){
    max_side_panel_width = 350;
}else{
...
function set_side_panel_width(c){
    if (c < minimum_side_panel_width || compact_side_panel_mode){
        if (c < 10){
            ...
            return;
        }else if (c < 120 && !compact_side_panel_mode){
            enable_animations(true);
            compact_side_panel_mode = true;
            application.style.setProperty('--side-panel-extended-width', 180 + "px");
            current_side_panel_size = 74;
            set_side_panel_compact(true);
            return;
        }else if (c > 180 && compact_side_panel_mode){
